...
import { Loader2, AlertCircle, Calendar, Hash, Tag, Clock, User } from 'lucide-react'
import { ClientPackageDetails } from '../../../hooks/clients/useClientPackages'
import { formatDate, formatTime } from '../../../utils/i18n'

interface ClientPackageDetailsModalProps {
...
export default function ClientPackageDetailsModal({
  packageDetails,
  ...
  error,
  onClose,
}: ClientPackageDetailsModalProps) {
    ...
    if (loading) {
      return (
        <div className="packages-status">
          ...
          <p>{t('client.packages.loadingDetails')}</p>
        </div>
      )
    }
      ...
      <div className="package-details-content">
        <div className="package-details-info">
          <div className="detail-row">
            <User size={16} />
            <span className="detail-label">{t('client.packages.professional')}:</span>
            <span className="detail-value">{professionalFirstName} {professionalLastName}</span>
          </div>
          <div className="detail-row">
            <Calendar size={16} />
            <span className="detail-label">{t('client.packages.issuedAt')}:</span>
            <span className="detail-value">{formatDateDisplay(packageDetails.issued_at)}</span>
          </div>
          <div className="detail-row">
            <Calendar size={16} />
            <span className="detail-label">{t('client.packages.expiresAt')}:</span>
            <span className="detail-value">{formatDateDisplay(packageDetails.expires_at)}</span>
          </div>
          <div className="detail-row">
            <Hash size={16} />
            <span className="detail-label">{t('client.packages.appointments')}:</span>
        ...
        {packageDetails.appointments && packageDetails.appointments.length > 0 ? (
          <div className="package-appointments-section">
            <h3 className="package-section-title">{t('client.packages.appointmentsList')}</h3>
          ...
          <div className="packages-status-inline">
            <p>{t('client.packages.noAppointmentsInPackage')}</p>
          </div>
        )}
      </div>
    )
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content modal-content-large" onClick={(e) => e.stopPropagation()}>
        <h2>{t('client.packages.packageDetails')}</h2>
        {renderContent()}
